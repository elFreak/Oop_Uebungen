    ...
    if (!mouseEvent.isConsumed()) {
      ...
      if (minDistance(x, y, 5.0) < 5.0) {
        ...
        for (int i = 0; i < lineListenerVector.size(); i++) {
          ( (LineListener) lineListenerVector.elementAt(i)).linePressed( (x - xOffset) / xScale, ( -y - yOffset) / yScale, xData, yData);
        }
        parent.repaint();
      }
    }
  ...
  public void mouseDragged(MouseEvent mouseEvent) {
    ...
    if (!mouseEvent.isConsumed()) {
      ...
      if (isSelected) {
        ...
        int y = mouseEvent.getY();
        for (int i = 0; i < lineListenerVector.size(); i++) {
          ( (LineListener) lineListenerVector.elementAt(i)).lineDragged( (x - xOffset) / xScale, ( -y - yOffset) / yScale, xData, yData);
        }
        parent.repaint();
      }
    }
  ...
  public void mouseReleased(MouseEvent mouseEvent) {
    ...
    if (!mouseEvent.isConsumed()) {
      ...
      if (isSelected) {
        ...
        isSelected = false;
        for (int i = 0; i < lineListenerVector.size(); i++) {
          ( (LineListener) lineListenerVector.elementAt(i)).lineReleased( (x - xOffset) / xScale, (y - yOffset) / yScale, xData, yData);
        }
        parent.repaint();
      }
    }
